...
# mm and cm -> m
# Change 2022:
# Added a new method to only retrieve the angle of the paired beacons (return_angle()-> 0-360º)
###
...
class MarvelmindHedge (Thread):
    ...
    def return_angle(self):
        ...
        print('Angulo REAL leido: ',((self.position()[4]-900)/10))
        angle=(self.position()[4]-900)/10
        if angle < 0:
            angle = angle+360
        return angle*(math.pi/180) #Angle located at position 5 (with Y correction) [rad]
